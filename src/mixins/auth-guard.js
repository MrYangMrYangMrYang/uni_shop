...
export default {
	...
	methods: {
		...
		navigateToLogin(fromPath) {
			// 区分 TabBar 页面（switchTab）和非 TabBar 页面（navigateTo），
			// 避免对 /subpkg/* 路由使用 switchTab 导致静默失败
			const isTabPage = fromPath && fromPath.startsWith('/pages/');
			const redirectInfo = {
				openType: isTabPage ? 'switchTab' : 'navigateTo',
				from: fromPath
			};
