...
import userModule from '@/src/store/user.js';
...
describe('store/user.js', () => {
	...
	describe('updateUserInfo', () => {
		it('更新用户信息', () => {
			const info = { avatarUrl: 'https://a.b/c.png', nickName: 'Sunny' };
			store.commit('m_user/updateUserInfo', info);
			expect(store.state.m_user.userinfo).toEqual(info);
		});
	});
	describe('updateToken', () => {
		it('设置 token', () => {
			store.commit('m_user/updateToken', 'Bearer xyz');
			expect(store.state.m_user.token).toBe('Bearer xyz');
		});
		it('清空 token', () => {
			store.commit('m_user/updateToken', 'Bearer old');
			store.commit('m_user/updateToken', '');
			expect(store.state.m_user.token).toBe('');
		});
	});
	describe('addOrder', () => {
		it('新订单添加到列表头部', () => {
			const order = { order_id: '1', status: 0, total_price: 1000 };
			store.commit('m_user/addOrder', order);
			expect(store.state.m_user.orderList).toHaveLength(1);
			expect(store.state.m_user.orderList[0].order_id).toBe('1');
		});
	});
	describe('updateRedirectInfo', () => {
		it('设置重定向信息', () => {
			store.commit('m_user/updateRedirectInfo', { openType: 'switchTab', from: '/pages/cart/cart' });
			expect(store.state.m_user.redirectInfo.from).toBe('/pages/cart/cart');
		});
		it('传 null 清除重定向信息', () => {
			store.commit('m_user/updateRedirectInfo', { from: '/x' });
			store.commit('m_user/updateRedirectInfo', null);
			expect(store.state.m_user.redirectInfo).toBeNull();
		});
	});
});
