...
无障碍示范页面：aria-label / aria-live 等属性标注了关键交互元素 * 为视障用户提供屏幕阅读器友好的浏览体验 */
<template>
	...
	<view v-else-if="goods_info.goods_name" class="goods-detail-container u-page u-page--page">
			...
			>
				<swiper-item v-for="(item, i) in goods_info.pics" :key="i">
					<u-image
						:src="item.pics_big"
						mode="aspectFill"
						@click.native="preview(i)"
						:aria-label="'商品图片 ' + (i + 1) + ' / ' + goods_info.pics.length"
					/>
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="goods-info-box u-card--shadow">
			<view class="price-row">
				...
				<view class="favi u-chip u-chip--outline u-pressable" role="button" aria-label="收藏商品">
...
import { getGoodsDetail } from '@/src/api/goods.js';
...
export default {
	...
	data() {
		return {
			...
			buttonGroup: [
				{ text: '加入购物车', backgroundColor: '#ffa200', color: '#fff' }, // $color-accent-gold
				{ text: '立即购买', backgroundColor: '#C00000', color: '#fff' } // $color-primary
			]
		};
	},
	...
	methods: {
		...
		buttonClick(e) {
			...
			if (!this.checkLogin('/subpkg/goods-detail/goods-detail?goods_id=' + this.goods_info.goods_id)) return;
