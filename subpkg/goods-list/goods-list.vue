			...
			<view class="filter-box">
				...
				>
					...
					<view class="filter-arrow" v-if="item !== '销量'">
						<uni-icons
							...
							size="12"
							:color="activeFilter === i ? filterActiveColor : filterMutedColor"
						></uni-icons>
					</view>
				</view>
				...
				<view class="filter-dropdown" v-if="showDropdown" @click.stop>
					...
					<block v-if="currentDropdownType === '综合'">
						...
						>
							综合排序
							<uni-icons type="checkmarkempty" size="14" :color="checkmarkColor" v-if="sortType === 'all'"></uni-icons>
						</view>
						<view
...
import { searchGoods } from '@/src/api/goods.js';
import { DEMO_BRAND_LIST, DEMO_SHOP_LIST } from '@/src/config/mock.js';
...
export default {
	...
	data() {
		return {
			filterActiveColor: '#C00000', // $color-primary
			filterMutedColor: '#909399', // $color-text-300
			checkmarkColor: '#C00000', // $color-primary
			queryObj: {
				query: '',
				cid: '',
				pagenum: 1,
			...
			currentDropdownType: '',
			// 演示品牌数据
			brandList: DEMO_BRAND_LIST,
			selectedBrand: '品牌',
			// 演示店铺数据
			shopList: DEMO_SHOP_LIST,
			selectedShop: '店铺'
		};
	},
	onLoad(options) {
	...
	onReachBottom() {
		if (this.queryObj.pagenum * this.queryObj.pagesize >= this.total) return showToast('已经到底啦~');
...
.filter-box {
	...
	.filter-dropdown {
		...
		box-shadow: 0 10rpx 30rpx $color-overlay-shadow;
